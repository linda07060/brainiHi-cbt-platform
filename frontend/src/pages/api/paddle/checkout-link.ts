...
/**
 * Proxy POST -> backend /billing/checkout-link
 * body: { plan: string, billingPeriod: 'monthly'|'yearly' }
 */
const BACKEND_BASE = process.env.NEXT_PUBLIC_API_URL || process.env.BACKEND_URL || "http://localhost:4001";
const BACKEND_PATH = "/billing/checkout-link";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.error("checkout-link proxy error", err);
